;(function ($) { //start with a [;] because if our code is combine or minification  with other code,AND other code not terminated with [;] then it will not infect ours.
    ...
    ** 使用 es6 的 发送事件方法
    **/
    function fireEvent(type,data,bubble){
        var evt = new Event(type,{bubbles:bubble||true});
        evt.data = data;
        dispatchEvent(evt);
    }
    ...
    Tabs.prototype.init = function () {
		...
		this.tabwrapper.find("li").click(function(e){
			if(!$(this).hasClass("active")){
				...
				fireEvent("tab_change",{index:idx,activeData:_this.config.list[idx]});
			}
		});
		...
		if(_this.config.rm){
			this.tabwrapper.find("li>a>i").click(function(e){
				...
				fireEvent("tab_removed",{rmData:_this.config.list[index],activeData:_this.config.list[aindex]});
			});
