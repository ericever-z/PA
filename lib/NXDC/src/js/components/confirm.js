	...
	**/
    ...
    Confirm.prototype.initConfig = function(){
		...
		if(cfg.title){
			...
			if(header) header.find(".top-title").html("");
		}
		...
		if(cfg.content){
			...
			if(body) body.append(icon).append(content);
		}else{
			body.empty();
		}
		
		if(cfg.icon){
			if(icon) icon.html(cfg.icon).addClass("showing");
			if(content) content.addClass("showing");
		}else{
